...
Paste a raw IT policy, a procedure, or upload a Cloud Security finding (like AWS Security Hub JSON), and the engine will suggest a mapping to the **Secure Controls Framework (SCF)** and list the regulations that SCF's own crosswalk links to the mapped controls (GDPR, SOC 2, ISO 27001, NIST, PCI).
...
Paste a raw IT policy, a procedure, or upload a large Cloud Security JSON (e.g. AWS Security Hub findings), and the LLM engine suggests the closest matching SCF domains and controls, with a confidence score, for human review.

```mermaid
graph TD
    A[Raw Input: Policy, Procedure, or Cloud Finding] --> B[Embedding similarity search: top 50 SCF candidates]
    B --> C(One LLM call selects the best matches)
    C --> D[Validation: unknown control IDs removed, confidence clamped]
    D --> E[Suggested SCF controls, for human review]
    E --> F[CSV export / Streamlit UI]
```

### 2. 🎯 Audit Scope Analyzer (Prototype)
Upload a narrative Audit Scope Document (TXT/PDF) and the AI will suggest which SCF Domains and baseline controls to test, for human review.
> [!TIP]
...
Upload a CSV listing your company's existing IT controls, select a target framework (e.g., SOC 2, HIPAA, GDPR), and it lists the SCF baseline controls that SCF's own crosswalk associates with that framework, for human review.
...
> **Note:** these samples are raw, unreviewed output from a small (8B) model and include weak matches. They show the format of the results, not their accuracy. The CSVs quote short excerpts of SCF control text with attribution (CC BY-ND 4.0).

## 🚀 Quickstart

1. Clone the repository and navigate to the project directory.
2. Get a FREE API Key from [Groq Console](https://console.groq.com/keys) to run Llama 3.1 8B (default).
